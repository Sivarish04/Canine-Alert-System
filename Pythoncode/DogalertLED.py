...
# ESP32-CAM IP to trigger the LED
esp32_led_url = 'http://10.42.0.197/blink'  # Replace with your ESP32-CAM IP address
...
while True:
    ...
    # Send a request to the ESP32-CAM to blink the LED if a dog is detected
    if dog_detected:
        print("Dog detected")
        try:
            requests.get(esp32_led_url)  # Send the HTTP request to blink the LED
        except requests.exceptions.RequestException as e:
            print(f"Error sending request to ESP32-CAM: {e}")
